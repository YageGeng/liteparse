...
///
/// YOLO predicts boxes in the square letterboxed image. This function removes
/// the padding, divides by the resize scale, clamps to the rendered image, and
/// finally scales into PDF points.
pub fn restore_box_to_page(
    ...
    // CONTEXT: Boxes may extend into padded areas or slightly beyond the
    // model image bounds. Clamp in rendered-image space before converting to
    // page points so downstream consumers never see negative dimensions.
    let image_x =
...
///
/// Candidates are sorted by confidence first, so when boxes overlap above the
/// threshold the highest-confidence candidate wins.
pub fn non_max_suppression(
    ...
    'candidate: for candidate in candidates {
        for existing in &kept {
            // CONTEXT: Suppression is class-agnostic because DocLayNet regions
            // should not occupy the same visual area with different labels.
...
mod tests {
    ...
    // Build a compact page-space candidate for non-max suppression tests.
    ...
    // Verifies that letterbox removal maps model boxes back into page coordinates.
    #[test]
    ...
    // Verifies that the highest-confidence overlapping box wins.
    #[test]
    ...
    // Verifies that suppression is class-agnostic for overlapping layout regions.
    #[test]
